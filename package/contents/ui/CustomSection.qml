...
ColumnLayout {
    ...
    Canvas {
        ...
        Connections {
            target: root
            function onCustomHistoryChanged()  { customGraph.requestPaint() }
            function onTextColorChanged()      { customGraph.requestPaint() }
            function onScrollTickChanged()     { if (root._custPhaseStart > 0 && root.custScrollPhase() < 2) customGraph.requestPaint() }
        }
        Connections {
            ...
            function onSmoothLinesChanged()    { customGraph.requestPaint() }
        }
        ...
        onPaint: {
            ...
            if (ct === 3) {
                ...
                cu.drawDonut(ctx, cx, cy, rad, Math.max(6, rad * 0.22),
                    Math.min(100, (root.customValue / maxVal) * 100), color,
            ...
            if (ct === 4) {
                const cx = yLW + gW/2, cy = height/2, rad = Math.min(gW, height) * 0.36
                cu.drawPie(ctx, cx, cy, rad, Math.min(100, (root.customValue / maxVal) * 100), color,
                    root.customValue.toFixed(1) + (plasmoid.configuration.customCmdUnit || ""),
                    plasmoid.configuration.customCmdTitle || "value")
                return
            }
            if (ct === 5) {
                ...
                cu.drawHorizontalBar(ctx, plasmoid.configuration.customCmdTitle || "Value",
                    (root.customValue / maxVal) * 100,
                    root.customValue.toFixed(2) + (plasmoid.configuration.customCmdUnit || ""),
                    color, bx, height/2 - barH/2, bw, barH)
                return
            }
            if (ct === 1) {
            ...
            cu.drawLine(ctx, h, color, iToX, valToY, height, smooth,
                ct === 2 ? 0.65 : 0.38,
                plasmoid.configuration.glowLine ? 8 : 0)

            // endpoint dot
            if (n > 0) {
                const lp = { x: iToX(n-1, n), y: valToY(h[n-1]) }
    ...
    RowLayout {
        ...
        LegendItem {
            ...
            color: plasmoid.configuration.customCmdColor || "#ffaa00"
            textColor: root.textColor; active: true
        }
        Item { Layout.fillWidth: true }
    }
}
