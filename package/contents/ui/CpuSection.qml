...
ColumnLayout {
    ...
    Canvas {
        ...
        Connections {
            ...
            function onCoreHistoriesChanged() { cpuGraph.requestPaint() }
            function onTextColorChanged()     { cpuGraph.requestPaint() }
            function onScrollTickChanged()    { if (root._cpuPhaseStart > 0 && root.cpuScrollPhase() < 2) cpuGraph.requestPaint() }
        }
        Connections {
            ...
            function onSmoothLinesChanged()   { cpuGraph.requestPaint() }
        }
        ...
        onPaint: {
            ...
            if (ct === 3) {
                ...
                if (!root.isLineDisabled("cpuTotal"))
                    cu.drawDonut(ctx, cx, cy, rad, lw, root.cpuPercent, root.cpuColor,
                ...
                if (plasmoid.configuration.showCpuCores) {
                    for (let ci = 0; ci < Math.min(root.corePercents.length, 8); ci++) {
                        ...
                        cu.drawDonut(ctx, cx, cy, cr, Math.max(2, lw * 0.30),
                            root.corePercents[ci] || 0,
                            root.coreColors[ci % root.coreColors.length] || "#888888", null, null)
                    }
                }
                return
            }
            if (ct === 4) {
                ...
                if (!root.isLineDisabled("cpuTotal"))
                    cu.drawPie(ctx, cx, cy, rad, root.cpuPercent, root.cpuColor,
                        root.cpuPercent.toFixed(1) + "%", "cpu")
                if (plasmoid.configuration.showCpuCores) {
                    for (let ci = 0; ci < Math.min(root.corePercents.length, 8); ci++) {
                        if (root.isCoreDisabled(ci)) continue
                        const cr = rad * (1.0 - (ci + 1) * 0.10)
                        if (cr < rad * 0.25) break
                        cu.drawPie(ctx, cx, cy, cr, root.corePercents[ci] || 0,
                            root.coreColors[ci % root.coreColors.length] || "#888888", null, null)
                    }
                }
                return
            ...
            if (ct === 5) {
                ...
                if (plasmoid.configuration.showCpuCores) {
                    ...
                    for (let ci = 0; ci < nc; ci++) {
                        ...
                        cu.drawHorizontalBar(ctx, "C" + (ci+1),
                            root.corePercents[ci] || 0,
                            (root.corePercents[ci] || 0).toFixed(0) + "%",
                            root.coreColors[ci % root.coreColors.length] || "#888888",
                            bx + col * (colW + 8), 5 + row * (barH + 6) + 8, colW, barH)
                    }
                } else if (!root.isLineDisabled("cpuTotal")) {
                    cu.drawHorizontalBar(ctx, "CPU Total", root.cpuPercent,
                        root.cpuPercent.toFixed(1) + "%", root.cpuColor, bx, height/2-7, bw, 14)
                }
                return
            }
            if (ct === 1) {
                if (plasmoid.configuration.showCpuCores) {
                    for (let ci = 0; ci < root.coreHistories.length; ci++) {
                        ...
                        cu.drawHistoryBars(ctx, root.coreHistories[ci],
                            root.coreColors[ci % root.coreColors.length] || "#888888",
                            yLW, gW, height, maxH, 100, 0)
                    }
                    ctx.globalAlpha = 1.0
                } else if (!root.isLineDisabled("cpuTotal")) {
            ...
            if (plasmoid.configuration.showCpuCores) {
                for (let ci = 0; ci < root.coreHistories.length; ci++) {
                    ...
                    ctx.lineWidth = isHov
                        ? plasmoid.configuration.lineWidth
                        : Math.max(0.6, plasmoid.configuration.lineWidth * 0.45)
                    cu.drawLine(ctx, ch,
                        root.coreColors[ci % root.coreColors.length] || "#888888",
                        iToX, pToY, height, smooth, 0, 0)
                    ctx.globalAlpha = 1.0
                }
            }
            ...
            if (n >= 2 && !root.isLineDisabled("cpuTotal")) {
                ...
                cu.drawLine(ctx, h, root.cpuColor, iToX, pToY, height, smooth, fillA,
    ...
    GridLayout {
        ...
        Repeater {
            ...
            delegate: Item {
                ...
                RowLayout {
                    anchors { fill: parent; leftMargin: plasmoid.configuration.showYLabels ? 40 : 6; rightMargin: 4 }
                    spacing: 5
                    Rectangle {
                    ...
                    Text {
                        ...
                        font.pixelSize: 11; font.strikeout: !coreItem.coreActive; elide: Text.ElideRight
                    }
                    Text {
                        text: (root.corePercents[index] || 0).toFixed(1) + "%"
                        color: coreItem.coreActive
                            ? coreItem.coreColor
                            : Qt.rgba(coreItem.coreColor.r, coreItem.coreColor.g, coreItem.coreColor.b, 0.4)
                        font.pixelSize: 11; font.bold: true
                        Layout.minimumWidth: 42; horizontalAlignment: Text.AlignRight
                    }
                }
            }
        }
