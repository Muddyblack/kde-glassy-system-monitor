...
ColumnLayout {
        ...
        Row {
            ...
            Repeater {
                ...
                delegate: Rectangle {
                    ...
                    width: Math.min(90, Math.max(36,
                        (pingSection.width - root.targetList.length * 4 - 80) / root.targetList.length))
                    height: 20; radius: height / 2
                    color: active
                        ? Qt.rgba(root.lineColor.r, root.lineColor.g, root.lineColor.b, 0.22)
                        : Qt.rgba(1, 1, 1, 0.06)
                    border.color: active
                        ? Qt.rgba(root.lineColor.r, root.lineColor.g, root.lineColor.b, 0.60)
                        : Qt.rgba(1, 1, 1, 0.14)
                    border.width: 1
    ...
    Canvas {
        ...
        Connections {
            target: root
            function onHistoriesChanged()      { pingGraph.requestPaint() }
            function onIsAlertingChanged()     { pingGraph.requestPaint() }
            function onLineColorChanged()      { pingGraph.requestPaint() }
            function onTextColorChanged()      { pingGraph.requestPaint() }
            function onScrollTickChanged()     { if (root._pingPhaseStart > 0 && root.pingScrollPhase() < 2) pingGraph.requestPaint() }
        }
        Connections {
            ...
            function onSmoothLinesChanged()      { pingGraph.requestPaint() }
        }
        ...
        onPaint: {
            ...
            if (n === 0) { cu.drawIdleLine(ctx, yLW, gW, height); return }
            ctx.setLineDash([])
            ...
            if (ct === 3) {
                ...
                cu.drawDonut(ctx, cx, cy, rad, Math.max(6, rad * 0.22), pct,
            ...
            if (ct === 5) {
                ...
                const pct  = root.lastPing >= 0 ? Math.min(100, (root.lastPing / maxMs) * 100) : 0
                cu.drawHorizontalBar(ctx,
                    root.targetList[root.activeTarget] || "Latency", pct,
                    root.lastPing >= 0 ? root.lastPing.toFixed(0) + " ms" : "— ms",
                    root.isAlerting ? "#ff6666" : root.lineColor,
                    bx, height / 2 - barH / 2, bw, barH)
                return
            }
            ...
            function iToX(i)   { return yLW + gW - (n - 2 - i + sf) * step }
            ...
            if (ct === 1) {
                ...
                for (let i = 0; i < n; i++) {
                    ...
                    if (h[i] < 0) {
                        ...
                        ctx.beginPath(); ctx.arc(x, height - tPad, 1.8, 0, Math.PI*2)
                    ...
                    if (bh > r*2) {
                        ...
                        ctx.lineTo(bx+barW,height-tPad); ctx.lineTo(bx,height-tPad); ctx.closePath()
            ...
            for (let i = 0; i < n; i++) {
                ...
                if (h[i] < 0) { if (seg.length) { segments.push(seg); seg = [] } }
            ...
            for (let i = 0; i < n; i++) {
                if (h[i] < 0) {
                    ...
                    if (x >= yLW - step/2 && x <= width + step/2) {
                        ctx.fillStyle = Qt.rgba(1, 0.15, 0.15, 0.08)
                        ctx.fillRect(x - step/2, tPad, step, height - tPad*2)
                        ctx.beginPath(); ctx.arc(x, height - tPad, 2, 0, Math.PI*2)
                        ctx.fillStyle = "#ff4444"; ctx.fill()
                    }
                }
            }
            ...
            for (const pts of segments) {
                ...
                function _iToX2(i, _len) { return pts[i].x }
                function _valToY(v)      { return msToY(v) }

                // draw line manually (segments have pre-computed x)
                ctx.save()
                if (plasmoid.configuration.glowLine) { ctx.shadowBlur = 8; ctx.shadowColor = sc }
                ctx.beginPath(); ctx.strokeStyle = sc; ctx.lineCap = "round"; ctx.lineJoin = "round"
            ...
            if (segments.length > 0) {
                ...
                if (lp) {
                    ctx.shadowBlur = plasmoid.configuration.glowLine ? 14 : 0
    ...
    RowLayout {
        ...
        Column {
            ...
            Text {
                readonly property var vh: (root.histories[root.activeTarget] || []).filter(v => v >= 0)
                text: vh.length > 0
                    ? Math.min.apply(null,vh).toFixed(0) + " / " + Math.max.apply(null,vh).toFixed(0) + " ms"
                    : "—"
    ...
    RowLayout {
        ...
        Item { width: plasmoid.configuration.showYLabels ? 38 : 0 }
        LegendItem { text: "Latency"; color: root.lineColor; textColor: root.textColor }
        Item { Layout.fillWidth: true }
    }
}
