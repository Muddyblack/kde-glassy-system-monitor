...
ColumnLayout {
    ...
    P5Support.DataSource {
        id: diskSource; engine: "executable"; connectedSources: []
    ...
    function parseDiskStats(text) {
        ...
        for (const line of text.split("\n")) {
            ...
            if (/[0-9]p[0-9]+$/.test(name)) continue   // nvme0n1p1
            if (/^sd[a-z]+[0-9]+$/.test(name)) continue // sda1
            if (/^vd[a-z]+[0-9]+$/.test(name)) continue // vda1
            ...
            if (activity > bestActivity) { bestActivity = activity; bestDisk = name }
        }

        const disk = (cfgDisk !== "auto" && diskData[cfgDisk]) ? cfgDisk : bestDisk
        if (!disk || !diskData[disk]) return

        const now = Date.now()
        const { rd, wr } = diskData[disk]
        ...
        if (lastDiskStats && lastDiskStats.disk === disk) {
            ...
            if (dt > 0.1) {
                ...
                const nw = wrHistory.slice(); nw.push(writeSpeed)
                if (nw.length > maxH) nw.splice(0, nw.length - maxH); wrHistory = nw
            }
        }
        lastDiskStats = { disk, rd, wr, time: now }
        activeDisk = disk
    ...
    Text {
        ...
        font.pixelSize: 9; visible: diskSection.activeDisk !== ""
    }
    ...
    Canvas {
        ...
        Connections {
            target: diskSection
            function onRdHistoryChanged()  { diskGraph.requestPaint() }
            function onWrHistoryChanged()  { diskGraph.requestPaint() }
        }
        Connections {
            target: root
            function onTextColorChanged()      { diskGraph.requestPaint() }
            function onHoveredLineChanged()    { diskGraph.requestPaint() }
            function onScrollTickChanged()     { if (root._dskPhaseStart > 0 && root.diskScrollPhase() < 2) diskGraph.requestPaint() }
        }
        Connections {
            target: plasmoid.configuration; ignoreUnknownSignals: true
            function onGlowLineChanged()     { diskGraph.requestPaint() }
            function onLineWidthChanged()    { diskGraph.requestPaint() }
            function onShowYLabelsChanged()  { diskGraph.requestPaint() }
            function onDiskRdColorChanged()  { diskGraph.requestPaint() }
            function onDiskWrColorChanged()  { diskGraph.requestPaint() }
            function onChartTypeChanged()    { diskGraph.requestPaint() }
            function onShowGridLinesChanged(){ diskGraph.requestPaint() }
            function onAutoYRangeChanged()   { diskGraph.requestPaint() }
            function onSmoothLinesChanged()  { diskGraph.requestPaint() }
        }
        ...
        onPaint: {
            ...
            if (rd.length < 1 && wr.length < 1) { cu.drawIdleLine(ctx, yLW, gW, height); return }
            ctx.setLineDash([])
            ...
            if (ct === 3) {
                ...
                if (!root.isLineDisabled("diskRd"))
                    cu.drawDonut(ctx, cx, cy, rad, lw,
                        Math.min(100, (diskSection.readSpeed  / maxBps) * 100), diskSection.rdColor,
                        "R " + cu.formatSpeed(diskSection.readSpeed),
                        "W " + cu.formatSpeed(diskSection.writeSpeed))
                if (!root.isLineDisabled("diskWr"))
                    cu.drawDonut(ctx, cx, cy, rad * 0.58, lw * 0.72,
                        Math.min(100, (diskSection.writeSpeed / maxBps) * 100), diskSection.wrColor, null, null)
                return
            }
            if (ct === 4) {
                const cx = yLW + gW/2, cy = height/2
                ...
                if (!root.isLineDisabled("diskRd"))
                    cu.drawPie(ctx, cx, cy, rad,
                        Math.min(100, (diskSection.readSpeed  / maxBps) * 100), diskSection.rdColor,
                        "R " + cu.formatSpeed(diskSection.readSpeed),
                        "W " + cu.formatSpeed(diskSection.writeSpeed))
                if (!root.isLineDisabled("diskWr"))
                    cu.drawPie(ctx, cx, cy, rad * 0.58,
                        Math.min(100, (diskSection.writeSpeed / maxBps) * 100), diskSection.wrColor, null, null)
                return
            }
            if (ct === 5) {
            ...
            if (ct === 1) {
                if (!root.isLineDisabled("diskRd")) cu.drawHistoryBars(ctx, rd, diskSection.rdColor, yLW, gW, height, maxH, maxBps, 0)
                if (!root.isLineDisabled("diskWr")) {
    ...
    RowLayout {
        ...
        Item {
            implicitWidth: rdRow.implicitWidth; implicitHeight: rdRow.implicitHeight
            Row {
                id: rdRow
                spacing: 5
                Rectangle {
                    ...
                    border.color: diskSection.rdColor; border.width: 1
                }
                Text {
                    text: "Read"
                    color: root.isLineDisabled("diskRd")
                    ...
                    font.pixelSize: 10; font.strikeout: root.isLineDisabled("diskRd")
                    anchors.verticalCenter: parent.verticalCenter
                }
                Text {
                    text: cu.formatSpeed(diskSection.readSpeed)
                    color: root.isLineDisabled("diskRd")
                        ? Qt.rgba(diskSection.rdColor.r, diskSection.rdColor.g, diskSection.rdColor.b, 0.3)
                        : diskSection.rdColor
            ...
            MouseArea {
                ...
                onClicked: { root.toggleLineDisabled("diskRd"); diskGraph.requestPaint() }
                onEntered: { root.hoveredLine = "diskRd"; diskGraph.requestPaint() }
                onExited:  { root.hoveredLine = "";        diskGraph.requestPaint() }
            }
        }
        ...
        Item {
            implicitWidth: wrRow.implicitWidth; implicitHeight: wrRow.implicitHeight
            Row {
                id: wrRow
                spacing: 5
                Rectangle {
                    ...
                    border.color: diskSection.wrColor; border.width: 1
                }
                Text {
                    text: "Write"
                    color: root.isLineDisabled("diskWr")
                        ? Qt.rgba(root.textColor.r, root.textColor.g, root.textColor.b, 0.3)
                        : Qt.rgba(root.textColor.r, root.textColor.g, root.textColor.b, 0.7)
                    font.pixelSize: 10; font.strikeout: root.isLineDisabled("diskWr")
                    anchors.verticalCenter: parent.verticalCenter
                }
                Text {
                    text: cu.formatSpeed(diskSection.writeSpeed)
                    color: root.isLineDisabled("diskWr")
                        ? Qt.rgba(diskSection.wrColor.r, diskSection.wrColor.g, diskSection.wrColor.b, 0.3)
                        : diskSection.wrColor
                    font.pixelSize: 12; font.bold: true
                    anchors.verticalCenter: parent.verticalCenter
                }
            ...
            MouseArea {
                anchors.fill: parent; cursorShape: Qt.PointingHandCursor; hoverEnabled: true
                onClicked: { root.toggleLineDisabled("diskWr"); diskGraph.requestPaint() }
                onEntered: { root.hoveredLine = "diskWr"; diskGraph.requestPaint() }
                onExited:  { root.hoveredLine = "";        diskGraph.requestPaint() }
            }
        }
    }
