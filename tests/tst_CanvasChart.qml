...
import QtQuick
import QtTest
import "../package/contents/ui/diagram" as Widget
...
Item {
    ...
    Component {
        ...
        Widget.CanvasChart {
            ...
            width: root.width
            height: root.height
            scrolling: settings.values.chartType < 3
            smoothScroll: settings.values.smoothScroll
            plotLeft: scrolling && settings.values.showYLabels ? 38 : 0
            historySize: settings.values.historySize
            bloom: settings.values.gpuBloom && settings.values.glowLine
            bloomStrength: settings.values.bloomStrength
            onScreen: root.fullRepVisible
            onPaintRequested: root.notePaintRequested()
            onPainted: root.notePainted()
    ...
    TestCase {
        ...
        function init() {
            root.phase = 0;
            root.scrollTick = 0;
            settings.values.gpuBloom = false;
            settings.values.chartType = 0;
            settings.values.showYLabels = false;
            settings.values.smoothScroll = true;
            root.width = 320;
            root.mainPaints = 0;
            root.glowPaints = 0;
            root.chromePaints = 0;
        ...
        function test_scrollingDoesNotPaintOverAxis() {
            settings.values.showYLabels = true;
            chart.requestPaint();
            wait(40);
            tick(1);
            const scrolled = grabImage(root);
        ...
        function test_disablingSmoothShowsCurrentSample() {
            tick(0.5);
            settings.values.smoothScroll = false;
            chart.requestPaint();
            wait(40);
